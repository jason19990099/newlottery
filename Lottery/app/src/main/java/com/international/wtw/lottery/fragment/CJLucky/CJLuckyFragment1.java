...
public class CJLuckyFragment1 extends BetBaseFragment implements BetItemAdapter.ItemBettingCallback, StateInterface {
    private View view;
...
//        requestOdds(mActivity.getLotteryType(), Constants.GD_HAPPY_PLAY_TYPE.DOUBLE_SIDE);
    ...
    private void setupLmpOddsView(List<View> lmpTopItemViews, List<NewOddsBean> currentOddBeans) {
        ...
        if (null == lmpTopItemViews || null == currentOddBeans || null == getActivity()) {
            return;
        }
        int size = lmpTopItemViews.size();
        int size1 = currentOddBeans.size();
        if (size != size1) {
            return;
        }

        for (int i = 0; i < lmpTopItemViews.size(); i++) {
                ...
                getActivity().runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (finalI == 0) {
                            ...
                            BetItemAdapter adapter = new BetItemAdapter(oneBean.getList(), getActivity(), CJLuckyFragment1.this, IsFeng);
    ...
    private void getBettingData() {
        ...
        betParams.put("type_code", String.valueOf(Constants.GD_HAPPY_PLAY_TYPE.DOUBLE_SIDE));
