...
public class HaveOrderFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener, View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        lv_have = (ListView) view.findViewById(R.id.lv_have);
        ...
        bet_is_win = getBetIsWin();
        LogUtil.e("bet_is_win-111----" + bet_is_win);
        SetData(time, page, 1, bet_is_win);

        return view;
    }

    public void setBetIsWin(int bet_is_win) {
        this.bet_is_win = bet_is_win;
        LogUtil.e("bet_is_win-222----" + bet_is_win);
        SetData(time, 1, 1, bet_is_win);
    }

    public int getBetIsWin() {
        return bet_is_win;
    }



    private void SetData(String datetime, int page, int flag, int bet_is_win) {
        String login_oid = SharePreferencesUtil.getString(getActivity(), LotteryId.Login_oid, null);
        HttpRequest.getInstance().getOutStandHave(getActivity(), login_oid, page, "20", "1", datetime, bet_is_win, new HttpCallback<SummaryDetailsBean>() {
            @Override
            public void onSuccess(SummaryDetailsBean data) {
                ...
                if (summaryDetailsResBeanList.size() != 0) {
                    hasAdapter = new HasAdapter(getActivity(), summaryDetailsResBeanList);
