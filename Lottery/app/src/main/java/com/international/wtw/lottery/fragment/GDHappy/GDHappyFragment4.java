...
public class GDHappyFragment4 extends BetBaseFragment implements StateInterface {
    ...
    private List<NewOddsBean.ListBean> lianMaOddsBeans = new ArrayList<>();//存放被投注的球的信息
    private List<NewOddsBean.ListBean> selectedBeans = new ArrayList<>(); //被选中的bean
    private List<View> lianMaTopItemView = new ArrayList<>();    //连码
...
//        requestOdds(mActivity.getLotteryType(), Constants.GD_HAPPY_PLAY_TYPE.JOINT_MARK);
    ...
    private List<NewOddsBean.ListBean> findTheLianMaSelectedBeans(List<NewOddsBean.ListBean> lianMaOddsBeans) {
        ...
        if (null != lianMaOddsBeans) {
            ...
            for (int i = 0; i < size; i++) {
                NewOddsBean.ListBean one = lianMaOddsBeans.get(i);
    ...
    protected void performBetting(@NonNull ArrayMap<String, String> paramMap) {
        paramMap.put(LotteryId.OID, SharePreferencesUtil.getString(getActivity(), LotteryId.Login_oid, ""));
        ...
        LogUtil.e("=====请求参数是=====" + (new JSONObject(paramMap)).toString());
        requestBet(body);
    }
    ...
    protected ArrayMap<String, String> getBetParam() {
        ...
        betParams.put(LotteryId.TYPE_CODE, String.valueOf(Constants.GD_HAPPY_PLAY_TYPE.JOINT_MARK));
