...
public class TeMaSxFragment extends BetBaseFragment implements BetColorsItemAdapter.OnItemClickListener, StateInterface {
    private View viewTmSx;
    private boolean isFeng;
    private List<NewOddsBean> teMaSXBeanList;   //请求到的赔率数据
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        viewTmSx = inflater.inflate(R.layout.view_six_bb, null);
        mFlOddsContainer.addView(viewTmSx);
        initViewTmSx();
        requestOddsInfo(16);
        return rootView;
    }
     ...
     * 初始化view
     */
    private void initViewTmSx() {
        tv_ball_name = (TextView) viewTmSx.findViewById(R.id.tv_ball_name);
        recycle_view = (RecyclerView) viewTmSx.findViewById(R.id.recycle_view_bb);
    }
...
//        requestOdds(mActivity.getLotteryType(), type_code);
    ...
    protected void onGetOdds(List<NewOddsBean> data) {
        teMaSXBeanList = data;
        setupTmSxOddsView(teMaSXBeanList);
        showOdds();
    }
     ...
     */
    private void setupTmSxOddsView(List<NewOddsBean> currentOddBeans) { //, int position
        ...
        if (null == adapter) {
            ...
            recycle_view.setLayoutManager(new GridLayoutManager(TeMaSxFragment.this.getActivity(), 3));
    ...
    private List<NewOddsBean.ListBean> findTheSelectedBeans() {
        return findTheSelectedBeans(teMaSXBeanList);
    }
    ...
    private void getBettingData() {
        ...
        betParams.put(LotteryId.TYPE_CODE, String.valueOf(16));
    ...
    public void clearBettingSelect( ) {
        ...
        if (adapter == null)
            return;
        adapter.refreshData(teMaSXBeanList.get(0).getList(),isFeng);
